<script lang="ts">
	...
	import type { EditUserDTO, GradeLevel, StrandDto, StudentFormData, UserDTO } from './type';
	import { editStudent, editUser, getGradeLevels, getStrands, getUserById } from './repo';
    ...
    let resultUser: CallResultDto<UserDTO>;
    ...
    try {
        if ($loggedInUser) {
            if ($loggedInUser.userTypeId == "1") {
                const result = await getStudentById(parseInt($loggedInUser.uid), true);

                if (result.isSuccess) {
                    student = {
                        ...result.data,
                        id: parseInt($loggedInUser.uid)
                    };
                ...
                resultUser = await getUserById(parseInt($loggedInUser.uid));
                user = {
                    ...resultUser.data,
                    id: parseInt($loggedInUser.uid),
                };
    ...
    async function handleSubmitUser(e: Event) {
		e.preventDefault();
		user = { ...user};
		try {
			if ($loggedInUser) {
				const callResult: CallResultDto<object> = await editUser(
					user
				);
								...
								<div class="column is-3">
								  <div class="field ">
									<div class="control">
									  ...
									  <select class="input has-background-white has-text-black" disabled bind:value={gradeLevel}>
									  ...
									  <div class="column is-3">
										<div class="field ">
										  ...
										  <select class="input has-background-white has-text-black" disabled bind:value={strandID}>
								  ...
								  <div class="field mb-4">
									<div class="control">
											...
											<div class="is-flex"><label class="label has-text-black">Birthdate</label>    <label class="label has-text-danger"> *</label></div>
											  <input class="input has-background-white has-text-black" type="date" bind:value={birthdate} required>
										  </div>
...
<div class="container is-narrow">
        ...
        <form class=" mx-auto" on:submit={handleSubmitUser}>
            <fieldset>
              ...
              <div class="block is-flex">
                    <div class="is-flex mr-4">
                        ...
                        <input type="checkbox" disabled bind:checked={user.active}>
                    </div>
