<script lang='ts'>
    ...
    import { getSchoolById, getSchoolYearList, getCurrentSchoolTerm,  getTotalBySection, getTotalBySchool, getLevelBySection } from "./repo";
...
return () => {
    ...
    try {
        if ($loggedInUser) {
            ...
            studentListCallResult = await getLevelBySection(parseInt($loggedInUser?.uid), sy);
