<script lang="ts">
	...
	import { editStudent, getGradeLevels, getStrands, getStudentById } from "../../repo";
...
onMount(async () => {
    try {
        const result = await getStudentById(data.id, false);
...
function formatBirthDay(date: Date, formatType: string = 'Date'): string {
    ...
    async function handleSubmit(e:Event){
       ...
       student.gradeLevelId =gradeLevel
         ...
         try {
          if($loggedInUser){
            const callResult:CallResultDto<object> = await editStudent(student, parseInt($loggedInUser.uid));
                        ...
                        <div class="column is-one-quarter">
                          <div class="field ">
                            <div class="control">
                              ...
                              <select class="input has-background-white has-text-black" bind:value={gradeLevel}>
                              ...
                              <div class="column is-one-quarter">
                                <div class="field ">
                                  ...
                                  <select class="input has-background-white has-text-black" disabled={student.gradeLevelId <= 13} bind:value={strandID}>
